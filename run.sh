...
#   ./run.sh --channels 21,22,23 --rate 5     # arm, from a flight computer address
#   ./run.sh --receive-only --dump 2          # receive, from the FSDR's address
...
# to the FSDR. Nothing owns both addresses, so --receive-only is its own process.
